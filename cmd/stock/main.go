...
func init() {
	flag.StringVar(&flagconf, "conf", "../../configs/stock", "config path")
}

func newApp(logger *slog.Logger, hs *http.Server) *kratos.App {
	return kratos.New(
		kratos.ID(id), kratos.Name("stock"),
...
func main() {
	...
	c := config.New(config.WithSource(file.NewSource(flagconf), env.NewSource("STOCK")))
	defer c.Close()
